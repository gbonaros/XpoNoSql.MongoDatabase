...
// This file implements the IDataStore bridge that executes MongoDB aggregation pipelines and basic CRUD as part of the XPO to MongoDB translation pipeline.
...
/// Supports select via aggregation and basic insert/update/delete using the MongoDB driver;
/// </summary>
public sealed partial class MongoDataStore : IDataStore
{
    ...
    private readonly ConcurrentDictionary<string, IMongoCollection<BsonDocument>> collectionCache = new(StringComparer.Ordinal);
    ...
    /// <summary>
    /// Performs schema updates honoring auto-create flags, creating collections and indexes when allowed.
    /// </summary>
    public UpdateSchemaResult UpdateSchema(bool doNotCreateIfFirstTableNotExist, params DBTable[] tables)
    {
        ...
        // For None/SchemaAlreadyExists, do not touch schema.
        if (autoCreateOption == AutoCreateOption.None || autoCreateOption == AutoCreateOption.SchemaAlreadyExists)
        {
            return UpdateSchemaResult.SchemaExists;
        }

        var first = tables[0];
        if (doNotCreateIfFirstTableNotExist && (first is null || !CollectionExists(first.Name)))
        {
            return UpdateSchemaResult.FirstTableNotExists;
        }

        bool allowCreateCollections = autoCreateOption == AutoCreateOption.DatabaseAndSchema ||
                                      (autoCreateOption == AutoCreateOption.SchemaOnly && DatabaseExists());
        ...
        foreach (DBTable table in tables)
        {
            ...
            if (!CollectionExists(table.Name))
            {
    ...
    private IMongoCollection<BsonDocument> GetCollection(string name, bool allowCreate = false)
    {
        if (string.IsNullOrEmpty(name))
            throw new ArgumentException("Collection name is required.", nameof(name));

        return collectionCache.GetOrAdd(name, key =>
        {
            var exists = CollectionExists(key);
            if (!exists)
            {
                bool canCreate = allowCreate && (
                ...
                {
                    throw new InvalidOperationException($"Collection '{key}' does not exist and auto-creation is disabled.");
                }
            }

            return database.GetCollection<BsonDocument>(key);
        });
    }
    ...
    private bool DatabaseExists()
    {
        try
        {
            // Use ListCollectionNames as a lightweight probe; avoids list-databases permission requirements.
    ...
    private bool EnsureIndexesForTable(IMongoDatabase db, DBTable table)
    {
        var coll = GetCollection(table.Name, allowCreate: true);
    ...
    public SelectedData SelectData(params SelectStatement[] selects)
    {
        ...
        if (selects != null)
        {
            foreach (var select in selects)
            {
                ...
                var collection = GetCollection(plan.RootCollection, allowCreate: false);
    ...
    {
        var collection = GetCollection(insert.Table.Name, allowCreate: false);
    ...
    {
        var collection = GetCollection(update.Table.Name, allowCreate: false);
    ...
    {
        var collection = GetCollection(delete.Table.Name, allowCreate: false);
    ...
    {
        var counters = GetCollection("__xpo_counters", allowCreate: true);
        ...
        if (result is null)
        {
            var col = GetCollection(insert.Table.Name, allowCreate: false);
