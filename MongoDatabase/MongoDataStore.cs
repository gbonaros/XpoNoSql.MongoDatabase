...
public sealed partial class MongoDataStore : IDataStore
{
    ...
    public UpdateSchemaResult UpdateSchema(bool doNotCreateIfFirstTableNotExist, params DBTable[] tables)
    {
        ...
        bool allowCreateCollections = autoCreateOption == AutoCreateOption.DatabaseAndSchema ||
                                      autoCreateOption == AutoCreateOption.SchemaOnly;
    ...
    private IMongoCollection<BsonDocument> GetCollection(string name, bool allowCreate = true)
    {
        if (string.IsNullOrEmpty(name))
            throw new ArgumentException("Collection name is required.", nameof(name));

        var exists = CollectionExists(name);
        if (!exists)
        {
            if (allowCreate &&
            ...
            {
                throw new InvalidOperationException($"Collection '{name}' does not exist and auto-creation is disabled.");
            }
        }
    ...
    {
        var coll = GetCollection(table.Name);
    ...
    public SelectedData SelectData(params SelectStatement[] selects)
    {
        ...
        if (selects != null)
        {
            foreach (var select in selects)
            {
                ...
                var collection = GetCollection(plan.RootCollection);
    ...
    {
        var collection = GetCollection(insert.Table.Name);
    ...
    {
        var collection = GetCollection(update.Table.Name);
    ...
    {
        var collection = GetCollection(delete.Table.Name);
    ...
    {
        var counters = GetCollection("__xpo_counters");
        ...
        if (result is null)
        {
            var col = GetCollection(insert.Table.Name);
